...
test('admin cria um grupo, empresta um papel e escolhe os membros', async ({ page }) => {
    ...
    await expect(editor.getByLabel('Chave')).toHaveValue('e2e-peritos');
    await editor.getByText('Leitor', { exact: true }).click();
    await editor.getByRole('button', { name: 'Salvar' }).click();

    const row = page.getByTestId('group-row-e2e-peritos');
    ...
    await expect(page.getByTestId('group-row-e2e-peritos')).toContainText('2 membros');
    const saved = state.groups.find((g) => g.key === 'e2e-peritos');
...
test('grupo sem papel avisa que não concede nada', async ({ page }) => {
    ...
    await expect(page.getByTestId('group-row-e2e-vazio')).toContainText('sem papéis — não concede nada');
});
...
test('remover o membro de um grupo tira o acesso que vinha por ele', async ({ page }) => {
    ...
    const row = page.getByTestId('group-row-equipe-regulacao');
    ...
    await expect(page.getByTestId('group-row-equipe-regulacao')).toContainText('0 membros');
    expect(state.groups.find((g) => g.key === 'equipe-regulacao').member_ids).toEqual([]);
});
